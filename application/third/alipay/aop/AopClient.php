...
class AopClient
{
    ...
    //使用文件读取文件格式，请只传递该值
    public $windowsPublicKeyFilePath = null;
    //使用读取字符串格式，请只传递该值
    public $windowsPublicKey;
    ...
    public function __construct($appId, $merchantPrivateKey, $merchantPublicKey, $windowsPublicKey, $apiVer, $signType)
    {
        $this->appId                      = $appId;
        $this->merchantPrivateKeyFilePath = $merchantPrivateKey;
        $this->merchantPublicKeyFilePath  = $merchantPublicKey;
        $this->windowsPublicKeyFilePath   = $windowsPublicKey;
    ...
    {
        if ($this->checkEmpty($this->merchantPrivateKeyFilePath)) {
            $priKey = $this->rsaPrivateKey;
            $res    = "-----BEGIN RSA PRIVATE KEY-----\n" . wordwrap($priKey, 64, "\n", true) . "\n-----END RSA PRIVATE KEY-----";
        } else {
            $priKey = file_get_contents($this->merchantPrivateKeyFilePath);
    ...
    */
    ...
    public function execute($request, $authToken = null, $appInfoAuthtoken = null, $getRequestUrl = false)
    {
        ...
        if (method_exists($request, "getNeedEncrypt") && $request->getNeedEncrypt()) {
            if ("json" == $this->format) {
                $resp = $this->_encryptJSONSignSource($request, $resp);
                // 将返回结果转换本地文件编码
                $r          = iconv($this->postCharset, $this->fileCharset . "//IGNORE", $resp);
                $respObject = json_decode($r);
            } else {
                $resp = $this->_encryptXMLSignSource($request, $resp);
                $r          = iconv($this->postCharset, $this->fileCharset . "//IGNORE", $resp);
     ...
     * @return bool
     */
    private function _verify($data, $sign)
    {
        if (is_array($data)) {
            $query = $this->getSignContent($data);
        } else {
            $query = $data;
        }
        //读取支付宝公钥文件
        $pubKey = file_get_contents($this->windowsPublicKeyFilePath);
        //转换为openssl格式密钥
        $res = openssl_get_publickey($pubKey);
        ($res) or die('支付宝RSA公钥错误。请检查公钥文件格式是否正确');
        //调用openssl内置方法验签，返回bool值
        $result = (bool)openssl_verify($query, base64_decode($sign), $res, OPENSSL_ALGO_SHA256);
        //释放资源
        openssl_free_key($res);
    ...
    /**
     * 网关验证，验证通过直接回复。验证失败返回false
     * @return bool
     */
    public function verifyGateway()
    {
        $xml = simplexml_load_string($_POST['biz_content']);
        if ((string)$xml->EventType == 'verifygw') {
            if (isset($_POST['sign'])) {
                $sign = $_POST['sign'];
                unset($_POST['sign']);

                if (!$this->_verify($_POST, $sign)) {
                    return false;
                }
                $response_xml  = "<biz_content>" . $this->getPublicKeyStr($this->merchantPublicKeyFilePath) . "</biz_content>";
                $response_xml .= "<success>true</success>";
    ...
    public function verifyMessage()
    {
        if (isset($_POST['sign'])) {
            ...
            return $this->_verify($_POST, $sign);
        }
        return false;
    }

    public function verifyPayNotify()
    {
        if (isset($_GET['sign'])) {
            $sign = $_GET['sign'];
            unset($_GET['sign']);
    ...
    {
        if ($this->checkEmpty($this->windowsPublicKeyFilePath)) {
            //读取字符串
            $pubKey = $this->windowsPublicKeyFilePath;
            $res    = "-----BEGIN PUBLIC KEY-----\n" . wordwrap($pubKey, 64, "\n", true) . "\n-----END PUBLIC KEY-----";
        } else {
            //读取公钥文件
            $pubKey = file_get_contents($this->merchantPublicKeyFilePath);
            //转换为openssl格式密钥
            $res = openssl_get_publickey($pubKey);
        }
    ...
    {
        if (!$this->checkEmpty($this->windowsPublicKeyFilePath) || !$this->checkEmpty($this->windowsPublicKeyFilePath)) {
            ...
            if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
                $checkResult = $this->_verify($signData->signSourceData, $signData->sign);
                if (!$checkResult) {
    ...
    private function _setupCharsets($request)
    {
        if ($this->checkEmpty($this->postCharset)) {
            $this->postCharset = 'UTF-8';
        }
        $str               = preg_match('/[\x80-\xff]/', $this->appId) ? $this->appId : print_r($request, true);
        $this->fileCharset = mb_detect_encoding($str, "UTF-8, GBK") == 'GBK' ? 'GBK' : 'UTF-8';
    }

    // 获取加密内容
    private function _encryptJSONSignSource($request, $responseContent)
    {
        $parsetItem = $this->_parserEncryptJSONSignSource($request, $responseContent);
    ...
    private function _parserEncryptJSONSignSource($request, $responseContent)
    {
        $apiName      = $request->getApiMethodName();
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
        ...
        if ($rootIndex > 0) {
            return $this->_parserEncryptJSONItem($responseContent, $rootNodeName, $rootIndex);
        } elseif ($errorIndex > 0) {
            return $this->_parserEncryptJSONItem($responseContent, $this->ERROR_RESPONSE, $errorIndex);
        } else {
            return null;
        }
    }

    private function _parserEncryptJSONItem($responseContent, $nodeName, $nodeIndex)
    {
        $signDataStartIndex = $nodeIndex + strlen($nodeName) + 2;
        $signIndex          = strpos($responseContent, "\"" . $this->SIGN_NODE_NAME . "\"");
        // 签名前-逗号
    ...
    // 获取加密内容
    private function _encryptXMLSignSource($request, $responseContent)
    {
        $parsetItem = $this->_parserEncryptXMLSignSource($request, $responseContent);
        $bodyIndexContent = substr($responseContent, 0, $parsetItem->startIndex);
        $bodyEndContent   = substr($responseContent, $parsetItem->endIndex, strlen($responseContent) + 1 - $parsetItem->endIndex);
        $bizContent       = decrypt($parsetItem->encryptContent, $this->encryptKey);
        return $bodyIndexContent . $bizContent . $bodyEndContent;
    ...
    private function _parserEncryptXMLSignSource($request, $responseContent)
    {
        $apiName      = $request->getApiMethodName();
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
        ...
        if ($rootIndex > 0) {
            return $this->_parserEncryptXMLItem($responseContent, $rootNodeName, $rootIndex);
        } elseif ($errorIndex > 0) {
            return $this->_parserEncryptXMLItem($responseContent, $this->ERROR_RESPONSE, $errorIndex);
        } else {
            return null;
        }
    }

    private function _parserEncryptXMLItem($responseContent, $nodeName, $nodeIndex)
    {
